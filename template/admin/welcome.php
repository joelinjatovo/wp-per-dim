...
$page_exists = isset( $tabs[ $current_page ] ) || has_action( 'nexway_welcomes_' . $current_page );

if ( ! $page_exists ) {
	wp_safe_redirect( admin_url( 'admin.php?page=nexway' ) );
	exit;
}
?>
<div class="wrap nexway-wrap">
    <h1><?php echo sprintf( __( 'Welcome to %s!', 'nexway' ), 'Nexway' ); ?></h1>
    ...
    <form method="<?php echo esc_attr( apply_filters( 'nexway_welcomes_form_method_tab_' . $current_page, 'post' ) ); ?>" id="mainform" action="" enctype="multipart/form-data">
        <nav class="nav-tab-wrapper nxw-nav-tab-wrapper">
            <?php
                ...
                do_action( 'nexway_welcomes_tabs' );
            ?>
        </nav>
        <?php
            self::show_messages();

            do_action( 'nexway_welcomes_' . $current_page );
        ?>
    </form>
</div>